...
import { Bell, Info, Shield, Zap, CheckCircle, Trash2, Clock, BookOpen, Trophy } from "lucide-react"
        ...
        try {
            ...
            if (res.ok) {
                ...
                const mapped = (json.data?.notifications || []).map((n: any) => {
                    ...
                    } else if (n.notification_type === 'achievement') {
                        icon = Trophy;
                        color = "text-amber-500 bg-amber-500/10";
                    } else if (n.notification_type === 'assignment') {
                        icon = BookOpen;
            ...
            <div className={cn("flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b pb-8", isLight ? "border-slate-200" : "border-slate-800")}>
                <div>
                    ...
                    <p className={cn("text-lg", isLight ? "text-slate-600 font-medium" : "text-slate-400")}>
                        Stay updated with your latest alerts and achievements.
                    </p>
                </div>
                <Button 
                    variant="outline" 
                    onClick={markAllAsRead}
                    className={cn("rounded-xl border shadow-sm transition-all shadow-blue-500/5", isLight ? "bg-white hover:bg-blue-50 border-slate-200" : "bg-white/5 hover:bg-white/10 border-white/10")}
                >
                    Mark all as read
                </Button>
            </div>
                    ...
                    notifications.map((n, i) => (
                        <motion.div
                            ...
                            className={cn(
                                ...
                                !n.isRead && (isLight ? "ring-2 ring-blue-500/10 bg-blue-50/30" : "ring-2 ring-blue-500/20 bg-blue-500/5")
                            )}
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                    <h3 className={cn("font-bold truncate text-lg", isLight ? "text-slate-900" : "text-white")}>
                                        {n.title}
                                    </h3>
                                    {!n.isRead && (
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <button 
                                        ...
                                        className={cn("text-xs font-bold hover:underline", isLight ? "text-blue-600" : "text-blue-400")}
                                    >
